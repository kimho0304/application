...
#define  LOG_TAG    "hello_c"
#define  LOGD(...)  __android_log_print(ANDROID_LOG_DEBUG, LOG_TAG, __VA_ARGS__)

/*static void JNICALL print_devs(libusb_device **devs)
{
    libusb_device *dev;
    int i = 0, j = 0;
    uint8_t path[8];
...
JNIEXPORT void JNICALL Java_com_example_application_Terminal_main(JNIEnv *env, jobject thiz)
{
    libusb_device **devs;
    int r;
    ssize_t cnt;
...
JNIEXPORT void JNICALL Java_com_example_application_Terminal_strings(JNIEnv *env, jobject thiz)
{
    int init, cnt;
    libusb_device *dev;
    libusb_device **devs;
    init = libusb_init(NULL);
    cnt = libusb_get_device_list(NULL, &devs);
    libusb_device_handle *devh;

    LOGD("libusb_init() is called");

    devh = libusb_open_device_with_vid_pid(NULL, 0x04d8, 0x000a);

    if(!devh){
        LOGD("libusb_open_device_with_vid_pid is failed");
        libusb_close(devh);
        libusb_exit(NULL);
    }
    ...
    LOGD("libusb_get_device_list() is called");
    if (cnt < 0){
        libusb_exit(NULL);
    }
    int i = 0, j = 0;
    ...
    char *buf = (char*)malloc(999);
    strcpy(buf, " "); // with the null terminator the string adds up to 10 bytes

    char* result = (char*)malloc(9999);
    int inx = 0;
    ...
    while ((dev = devs[i++]) != NULL) {
        ...
        if (r < 0) {
            ...
            strcpy(buf, "failed to get device descriptor");
            jstring jstrBuf_ = (*env)->NewStringUTF(env, buf);
            //return jstrBuf_;
        }
        LOGD("%04x:%04x (bus %d, device %d)",desc.idVendor, desc.idProduct, libusb_get_bus_number(dev), libusb_get_device_address(dev));
    ...
    char* result2 = (char*)malloc(9999);
    strcpy(result2, "failed to get device descriptor");
    jstring jstrBuf = (*env)->NewStringUTF(env, result2);
    //return jstrBuf;
}


jstring JNICALL Java_com_example_application_Terminal_example(JNIEnv *env, jobject thiz)

{
    char *buf = (char*)malloc(999);
    strcpy(buf, "l"); // with the null terminator the string adds up to 10 bytes
    jstring jstrBuf = (*env)->NewStringUTF(env, buf);
    return jstrBuf;
}

JNIEXPORT jstring JNICALL Java_com_example_application_Terminal_test(JNIEnv* env, jobject obj)
{
    if( libusb_init(NULL) ) {
        LOGD("libusb_init() is called");
        return (*env)->NewStringUTF(env, "Library is loaded successfully!");
    }
    else
...
JNIEXPORT jint JNICALL Java_com_example_application_Terminal_unrootedUsbDescription(JNIEnv* env, jobject obj, int fileDescriptor)
{
...
JNIEXPORT jstring JNICALL Java_com_example_application_Terminal_set_the_native_Descriptor(int fileDescriptor) {
    libusb_context *ctx;
    libusb_device_handle *devh;
